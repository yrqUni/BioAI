...
parser.add_argument("--evalBatch", default=1000, type=int)
parser.add_argument("--visBatch", default=100, type=int)
...
logging.info('sample data info {} {}'.format(data['label'].mean(),data.shape))
sample = list(data['feature'])
label = list(data['label'])
if args.maxLenTrain==-1 or args.maxLenTest==-1:
    len_sample = [len(list(sample.split(' '))) for sample in list(data['feature'])]
    mean_len_sample = sum(len_sample)/len(len_sample)
    if args.maxLenTrain==-1:
        args.maxLenTrain=int(mean_len_sample)
    if args.maxLenTest==-1:
        args.maxLenTest=int(mean_len_sample*2)
logging.info('train sample len {}, test sample len {}\n'.format(args.maxLenTrain,args.maxLenTest))
...
for train_index, test_index in kf.split(sample):
    ...
    trainLoader = DataLoader(trainData, batch_size=args.train_batch_size, shuffle=True, num_workers=8)
    testLoader = DataLoader(testData, batch_size=args.test_batch_size, shuffle=True, num_workers=8)
    ...
    for epoch in range(args.epochs):
        ...
        total_train_loss = 0
        total_auc = 0
        for data in tqdm(trainLoader, desc='Train {}/{}F {}/{}E'.format(k+1,args.kFold,epoch,args.epochs)):
            ...
            if (allBatchNum%args.visBatch)==0 or allBatchNum%len(trainLoader)==0:
                logging.info('train f{}/{} batch{}/{} loss: {:.3f}, auc: {:.3f}'.format(k+1,
                                                                                        args.kFold,
                                                                                        allBatchNum,
                                                                                        len(trainLoader)*args.epochs,
                                                                                        total_train_loss/(len(trainLoader)+1),
                                                                                        total_auc/(len(trainLoader)+1)))
            if (allBatchNum%(args.evalBatch/(epoch+1)))==0 or allBatchNum%len(trainLoader)==0:
                total_test_loss = 0
                total_auc = 0
                with torch.no_grad():
                ...
                logging.info('val f{}/{} batch{}/{} loss: {:.3f}, auc: {:.3f}\n'.format(k+1,
                                                                                        args.kFold,
                                                                                        allBatchNum,
                                                                                        len(trainLoader)*args.epochs,
                                                                                        total_test_loss/(len(testLoader)+1),
                                                                                        total_auc/(len(testLoader)+1)))
